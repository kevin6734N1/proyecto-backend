...
`informes_tecnicos.pdf_cargado` y `fecha_carga_pdf` son metadatos; el PDF firmado real está en `./data/pdf-firmados/{id}.pdf`. Se guarda una sola vez al subirlo por API, y aprobar el informe exige que ese archivo exista. Respaldar la carpeta junto con `gesmin.mv.db`; restaurar solo la base dejaría informes sin su archivo. Cotizaciones, órdenes e informes sin firma se guardan como snapshots en `./data/pdf-generados/` al crear cada registro: `cotizacion-{id}.pdf`, `orden-trabajo-{id}.pdf` e `informe-tecnico-{id}-sin-firma.pdf`. No hay columna nueva: la ruta se deriva de tipo e ID. Si falta el archivo (dato antiguo, fallo de escritura o borrado), el GET lo regenera al vuelo con los datos actuales de la BD y lo devuelve **sin persistirlo de nuevo** en `./data/pdf-generados/`; el snapshot original no se restaura. Respaldar esta carpeta junto con la base y los firmados; un cambio de estado posterior no regenera el snapshot. Si se reinicia la base de datos desde cero, hay que reiniciar o restaurar la carpeta de PDFs generados en conjunto: el nombre depende del ID y un ID reutilizado podría apuntar a un snapshot antiguo. La API rechaza un archivo residual cuando el registro ya no existe, pero esa comprobación no distingue un ID reutilizado.

### Registro del envío al cliente (convención provisional de Kevin, 25-set-2026)

Tras descargar el PDF firmado aprobado, Ventas lo envía por Gmail fuera del sistema y luego marca el informe `ENVIADO` mediante la API. `informes_tecnicos.fecha_envio` se asigna con la fecha actual del servidor al hacer ese cambio de estado; la descarga no modifica el estado ni la fecha. La BD no guarda destinatario, identificador del correo, hora efectiva ni constancia de entrega. Por ello, `ENVIADO` y `fecha_envio` representan el registro manual del operador, no una confirmación de Gmail. El PATCH no permite indicar una fecha histórica: para reflejar el día real, se debe marcar el mismo día del envío. No se agrega tabla ni columna para este paso; el procedimiento queda pendiente de confirmación por Gesmin.
